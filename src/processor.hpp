...
private:
    ...
    void tokenize_input(const std::string &input_sequence) {
        ...
        for (int i = 0; i < input_sequence.size(); i++) {
            if (input_sequence[i] == ' ') continue;
            else if (input_sequence[i] == 'x') output_sequence.push_back(Token(input_sequence[i]));
        ...
        if (bracket_quantity != 0) throw new 
                exceptions::BracketSequenceException(const_cast<char * >("Invalid bracket sequence"));
    }

    function_iterator get_valid_function(std::size_t current, const std::string& sequence) const noexcept {
        function_iterator retval = available_functions.end();
        for (int j = 1; (j < 4) && (j < sequence.size() - current) && (retval == available_functions.end()); j++) {
